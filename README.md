...
| `participants[].birthYear` | Optional. Sets the participant's workout-intensity zones (Rest / Z1: Recovery / Z2: Endurance / Z3: Aerobic / Z4: Threshold / Z5: Peak / SUPERMAX above HRmax) using the Tanaka max-heart-rate formula (208 − 0.7 × age). Zones follow the standard five-zone model: divisors at 50 / 60 / 70 / 80 / 90 / 100 % of HRmax. With neither `birthYear` nor `maxHr` set, zones assume age 65 and the overlay/setup pages show a warning. |
